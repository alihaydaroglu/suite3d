...
def get_crop(job, npil_filt_xy_um=5, cell_filt_xy_um=5,cell_filt_z_um=15, cft="gaussian"):
    crop = ((0,13), (130,445), (10, 440))
    return job.get_registered_movie('registered_fused_data', 'fused')[:, :, crop[1][0]:crop[1][1], crop[2][0]:crop[2][1]]
    # print(mov.shape)
...
def main():
    ...
    mov = get_crop(job)
    job.calculate_corr_map(mov=mov)
