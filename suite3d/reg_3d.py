...
def est_sub_pixel_shift(r, np = np):
    ...
    sub_pixel = (r[(center + 1) % max] - r[center - 1]) / ( 2 * r[center] - r[center - 1] - r[(center + 1) % max])
...
def process_phase_corr_per_frame(phase_corr, pc_size):
    ...
    z_sub_pixel = est_sub_pixel_shift(phase_corr_shifted[:, pc_peak_loc[1], pc_peak_loc[2]])
    x_sub_pixel = est_sub_pixel_shift(phase_corr_shifted[pc_peak_loc[0], :, pc_peak_loc[2]])
    y_sub_pixel = est_sub_pixel_shift(phase_corr_shifted[pc_peak_loc[0], pc_peak_loc[1], :])
...
def rigid_3d_ref_cpu(mov_cpu, mult_mask, add_mask, refs_f, pc_size, rmins = None, rmaxs = None, crosstalk_coeff = None):
    ...
    for t in range(nt):
        ...
        phase_corr_shifted[t], int_shift[t], pc_peak_loc[t], sub_pixel_shifts[t] = process_phase_corr_per_frame(phase_corr_tmp, pc_size)
    ...
    for b in range(total_batches):
        ...
        if process_mov:
            ...
            if mov_cpu_processed is None:
                ...
                mov_cpu_processed = n.zeros((mov_cpu_processed_tmp.shape[0], nt, mov_cpu_processed_tmp.shape[2], mov_cpu_processed_tmp.shape[3]), n.float32)
        ...
        phase_corr_shifted[t1:t2], int_shift[t1:t2], pc_peak_loc[t1:t2], sub_pixel_shifts[t1:t2] = process_phase_corr_gpu(phase_corr_tmp, cp.asarray(pc_size))
...
def shift_gpu(mov_gpu, shift_batch):
    ...
    for t in range(ntb):
        mov_gpu[:, t,:,:] = cp.roll(mov_gpu[:, t,:,:], (shift_batch[t ,1], shift_batch[t ,2]), axis = (1,2))
...
def reg_3d_gpu(mov_batch_gpu, fft_3d_ref_conj):
    ...
    for t in range(nt):
        fft_3d_mov[:,t,:,:] =  fft_3d_mov[:,t,:,:] / (1e-5 + cp.abs(fft_3d_mov[:,t,:,:]))
...
def process_phase_corr_gpu(phase_corr, pc_size):
    ...
    for t in range(nt):
        ...
        y_sub_pixel[t] = est_sub_pixel_shift(phase_corr_shifted[t, pc_peak_loc[t, 0], :, pc_peak_loc[t, 2]])
        x_sub_pixel[t] = est_sub_pixel_shift(phase_corr_shifted[t, pc_peak_loc[t, 0], pc_peak_loc[t, 1], :])
...
def shift_mov_fast(mov, shifts, fill_value = 0):
    ...
    for t in range(nt):
        ...
        if (shift[1] > 0) & (shift[2] > 0): #++
            ...
            shifted_mov[:, t, shift[1]:, shift[2]:] = mov[:, t, :-shift[1], :-shift[2]]
