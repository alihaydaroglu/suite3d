...
def load_and_stitch_tifs(paths, planes, verbose=True,n_proc=15, mp_args = {}, filt=None, concat=True, n_ch=30, convert_plane_ids_to_channel_ids = True, log_cb=default_log, debug=False, lbm=True, num_colors=1, functional_color_channel=0):
    ...
    if not(lbm):
        # use 
        return load_and_stitch_tifs_notLBM(paths, 
                                          planes, 
                                          verbose=verbose, 
                                          filt=filt, 
                                          concat=concat,
...
def load_and_stitch_tifs_notLBM(paths, planes, verbose=True, filt=None, concat=True, log_cb=default_log, debug=False, num_colors=1, functional_color_channel=0):
    '''
    ...
    for tif_path in paths:
        ...
        if num_colors>1:
            ...
            assert len(tif_file.shape)==4 and tif_file.shape[1]==num_colors, "tiffs have different number of imaging channels than indicated"
