...
def calculate_corrmap(
    ...
    log_cb(
        "Using conv_filt: %s, %.2f, %.2f" % (conv_filt_type, conv_filt_z, conv_filt_xy), 1
    )
    log_cb(
        "Using np_filt: %s, %.2f, %.2f" % (npil_filt_type, npil_filt_z, npil_filt_xy), 1
    )
    ...
    if type(mov) == dict:
        ...
        if nt < nz:
            ...
            flip_shape = False
            log_cb(
                "Shape is unexpected (%s). Modifying such that nt: %d and nz: %d"
                % (str(mov.shape), nt, nz)
            )
    ...
    for batch_idx in range(n_batches):
        ...
        log_cb(
            "Will process %d frames (%d-%d, t_batch_size: %d)"
            % (end_idx - st_idx, st_idx, end_idx, t_batch_size),
            3,
        )
        if reconstruct_svd:
            recon_tic = time.time()
            ...
            movx = svu.reconstruct_overlapping_movie(
                svd_info, t_indices=(st_idx, end_idx), n_comps=n_comps, crop_z=crop
            )
            log_cb(
                "Reconstructed %s mov in %.2f seconds"
                % (str(movx.shape), time.time() - recon_tic),
                3,
            )
        else:
            if flip_shape:
...
def calculate_corrmap_for_batch(
    ...
    mean_img[:] = (
        mean_img * (n_frames_proc - nt) / n_frames_proc
        + mov.mean(axis=0) * nt / n_frames_proc
    )
    ...
    det3d.np_sub_and_conv3d_split_shmem(
        ...
        unif_filt_size,
        n_proc=n_proc,
        batch_size=mproc_batchsize,
        np_filt_type=np_filt_type,
        conv_filt_type=conv_filt_type,
    )
    if mov_sub_save_path is not None:
        n.save(mov_sub_save_path, mov_sub.astype(dtype))
    log_cb("Vmap", 3)
    vmap2 += det3d.get_vmap3d(
        mov_filt,
        intensity_thresh,
        sqrt=False,
        mean_subtract=False,
        fix_edges=fix_vmap_edges,
    )
    if return_mov_filt:
