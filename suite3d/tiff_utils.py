...
def get_meso_rois(tif_path, max_roi_width_pix=145, find_common_z=True):
    ...
    for roi in si_rois:
        ...
        if roi_dict["pixXY"][0] > max_roi_width_pix and not warned:
            print(
                "SI ROI pix count in x is %d, which is impossible, setting it to %d"
...
@deprecated(
    "Not used anywhere, and it doesn't include the planes argument in the call to load_and_stitch_tifs"
)
def separate_planes_and_save(
    save_path,
    tif_paths,
    channels,
    ram_fraction=0.5,
    ram_cap_bytes=None,
    max_out_file_size_gb=3.0,
    tifs_per_file=None,
    n_proc_saving=12,
    crop=None,
    dir_per_plane=True,
    ...
    if tifs_per_file is None:
        ...
        print(
            "Capping at %.2f GB of %.2f GB available RAM"
            % (use_ram // (1024**3), free_ram // (1024**3))
        )

        max_n_blocks_per_file = max_out_file_size_gb * (1024**3) // block_size_per_plane
    ...
    print(
        "Loading %d files at a time, %d channels, for a total of %.2f GB"
        % (n_blocks, n_planes, n_blocks * block_size_loaded / 1024**3)
    )
    print(
        "Saving %d output files at a time, %.2f GB each for a total of %.2f GB"
        % (
            n_planes,
            block_size_per_plane * n_blocks / 1024**3,
            n_planes * block_size_per_plane * n_blocks / 1024**3,
        )
    )
    ...
    for iter_idx in range(n_iters):
        ...
        mov = lbmio.load_and_stitch_tifs(
            tif_paths[tif_idx : tif_idx + int(n_blocks)],
            verbose=True,
            channels=channels,
            filt=filt,
        )
        ...
        if n_proc_saving == 1:
            for plane_idx in range(n_planes):
                ...
                if dir_per_plane:
                    plane_dir = os.path.join(save_path, "plane%02d" % plane_idx)
            ...
            if dir_per_plane:
                for plane_idx in range(n_planes):
                    os.makedirs(
                        os.path.join(save_path, "plane%02d" % plane_idx), exist_ok=True
                    )
                output = p.starmap(
                    save_plane_worker,
                    [
                        (
                            os.path.join(
                                save_path,
                                "plane%02d" % plane_idx,
                                "plane%02d-%05d.tif" % (plane_idx, iter_idx),
                            ),
                            mov[plane_idx],
                        )
                        for plane_idx in range(n_planes)
                ...
                output = p.starmap(
                    ...
                    [
                        (
                            os.path.join(
                                save_path, "plane%02d-%05d.tif" % (plane_idx, iter_idx)
                            ),
                            mov[plane_idx],
                        )
                        for plane_idx in range(n_planes)
    ...
    **kwargs,
    ...
    for i in range(nFrames):
        if titles is None:
            ...
            im = animate_frame(
                Mov[i], ax, i, vminmax=vminmaxFrame, frame_title=titles[i], **kwargs
            )
            ims.append(im)
...
def show_tif_all_planes(
    ...
    **kwargs,
    ...
    fig, axs = plt.subplots(
        nrows, ncols, figsize=(figsize), layout="constrained", squeeze=False
    )
