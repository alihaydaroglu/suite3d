...
def nonrigid_2d_reg_gpu(
    ...
    if rmins is not None and rmaxs is not None:
        ...
        for zidx in range(nz):
            mov_gpu[:, zidx] = clip_and_mask_mov(
                mov_gpu[:, zidx], rmins[zidx], rmaxs[zidx]
            )
    ...
    smooth_t = time.time()
    pc, snrs = compute_snr_and_smooth(
        phase_corr, smooth_mat, n_smooth_iters, snr_thresh, log_cb=log_cb
    )
    # log_cb("Computed SNR and smoothed phase corrs in %.2f sec" % (time.time() - smooth_t),4)

    shift_t = time.time()
    ymaxs, xmaxs = get_subpixel_shifts(
...
def rigid_2d_reg_gpu_from_existing_shifts(
    ...
    load_t = time.time()
    log_cb(
        "Loaded mov and masks to GPU for rigid reg in %.2f sec" % ((load_t - start_t),),
        4,
    )
    ...
    if fuse_and_pad:
        ...
        log_cb(
            "GPU Mov of shape %d, %d, %d, %d; %.2f GB"
            % (nz, nt, ny, nx, mov_gpu.nbytes / (1024**3)),
            4,
        )
        mempool.free_all_blocks()
        log_cb(log_gpu_memory(mempool), 4)
    ...
    for zidx in range(nz):
        ...
        for frame_idx in range(nt):
            mov_shifted[frame_idx, zidx] = shift_frame(
                mov_shifted[frame_idx, zidx], dy=ymax_z[frame_idx], dx=xmax_z[frame_idx]
            )
...
def rigid_2d_reg_cpu(
    ...
    for zidx in range(nz):
        mov[zidx] = clip_and_mask_mov(
            mov[zidx], rmins[zidx], rmaxs[zidx], mult_mask[zidx], add_mask[zidx], cp=n
        )
        mov[zidx] = convolve_2d_cpu(mov[zidx], refs_f[zidx])
        unwrap_fft_2d(mov[zidx].real, max_reg_xy, out=phase_corr, cp=n)
        ymaxs[zidx], xmaxs[zidx], cmaxs[zidx] = get_max_cc_coord(
        ...
        if shift:
            for frame_idx in range(nt):
                mov_shifted[frame_idx, zidx] = shift_frame(
                    mov_shifted[frame_idx, zidx],
                    dy=ymaxs[zidx, frame_idx],
                    dx=xmaxs[zidx, frame_idx],
                    cp=n,
                )
...
def unwrap_fft_2d(mov_float, nr, out=None, cp=cp):
    ...
    idxs_out[-2] = slice(0, nr)
    idxs_out[-1] = slice(0, nr)
    idxs_mov[-2] = slice(-nr, ny)
    idxs_mov[-1] = slice(-nr, nx)
    # print(nr, ny)
    # print(idxs_out)
    # print(idxs_mov)
    ...
    idxs_out[-2] = slice(nr, ncc)
    idxs_out[-1] = slice(0, nr)
    idxs_mov[-2] = slice(0, nr + 1)
    idxs_mov[-1] = slice(-nr, nx)
    out[tuple(idxs_out)] = mov_float[tuple(idxs_mov)]
    idxs_out[-2] = slice(0, nr)
    idxs_out[-1] = slice(nr, ncc)
    idxs_mov[-2] = slice(-nr, ny)
    idxs_mov[-1] = slice(0, nr + 1)
    out[tuple(idxs_out)] = mov_float[tuple(idxs_mov)]
    idxs_out[-2] = slice(nr, ncc)
    idxs_out[-1] = slice(nr, ncc)
    idxs_mov[-2] = slice(0, nr + 1)
...
def compute_snr_and_smooth(
    phase_corr, smooth_mat, n_smooth_iters=2, snr_thresh=1.2, log_cb=default_log
):
    pc = phase_corr.copy()
    pc_smooth = pc.copy()
            ...
            for zx in range(nz):
                for bx in range(nb):
                    phase_corr[
                        ...
                        zx,
                        bx,
                        ymin[tx, zx, bx] : ymax[tx, zx, bx],
                        xmin[tx, zx, bx] : xmax[tx, zx, bx],
                    ] = 0
    else:
...
def crop_maxs(pc, npad, kernel=None, n_thread_per_block=512):
    ...
    xmin[xmin < 0] = 0
    xmax = xmaxs + npad + 1
    xmax[xmax > ncc_nopad] = ncc_nopad
    ymin = ymaxs - npad
    ymin[ymin < 0] = 0
    ymax = ymaxs + npad + 1
    ymax[ymax > ncc_nopad] = ncc_nopad
