...
def compute_reference_and_masks(mov_fuse, reference_params, log_cb = default_log, rmins = None, rmaxs = None, use_GPU = True):
    """
    ...
    if use_GPU:
        ref_image, ymax, xmax, cmax, used_frames = \
            get_reference_img_gpu(mov_fuse, percent_contribute, niter, rmins = rmins, rmaxs = rmaxs, batch_size = batch_size, max_reg_xy = max_reg_xy, sigma = sigma)
    else:
        ref_image, ymax, xmax, cmax, used_frames = \
...
def get_phasecorr_and_masks(ref_image, reference_params):
    ...
    refs_fft_NR = np.zeros((nblocks[0]*nblocks[1], nz, 1, block_size[0], block_size[1]), dtype = complex)
    for z in range(nz):
        mult_mask_NR[:,z,:,:], add_mask_NR[:,z,:,:], refs_fft_NR[:,z,:,:] = phasecorr_reference(
            refImg0=ref_image[z], maskSlope= smooth_sigma*3, smooth_sigma=smooth_sigma, yblock=yblock, xblock=xblock)
...
def init_ref_3d(frames):
    ...
    frames = np.reshape(frames, (nimg, -1)).astype('float32') #flattend all spatial dimensions
    frames -= np.reshape(frames.mean(axis = 1), (nimg,1)) #subtract mean intestity at each time
    ...
    imax = np.argmax(bestCC) #choose the time frame, which is most correlated with the other frames
    indsort = np.argsort(-cc[imax, :]) #gets the correlation if imax with all the other time points
    refImg = np.mean(frames[indsort[0:20], :], axis=0) #chose 20 most correlated images to the highly correlated img as the initial ref
...
def mean_centered_meshgrid3D(nz,ny,nx):
    ...
    zz, yy, xx = np.meshgrid(z,y,x, indexing='ij')  # change order here for ideal shape of mask
    return zz, yy, xx
...
def phasecorr_ref(RefImg, smooth_sigma = None):
    ...
    cfRefImg = n.conj(mkl_fft.fft2(RefImg)) #padding?  , (next_fast_len(ny), next_fast_len(nx))))
    #normalise cfRefImg
...
def align_planes(mov3D, reference_params):
    ...
    for zidx in range(1 , nz):
        mov3D[zidx] = reg.clip_and_mask_mov(mov3D[zidx], None, None, # can speed thisup with numba/parallelisation?
                            mult_mask[zidx], add_mask[zidx], cp=n)
        mov3D[zidx] = reg.convolve_2d_cpu(mov3D[zidx], refs_f[zidx-1]) # here is zidx and zidx - 1
        reg.unwrap_fft_2d(mov3D[zidx].real, max_reg_xy, out=phase_corr, cp=n)
        ymaxs[zidx], xmaxs[zidx], cmaxs[zidx] = reg.get_max_cc_coord(phase_corr, max_reg_xy, cp=n)
...
def apply_plane_shifts3D(mov, tvecs):
    ...
    for i in range(nz):
        shifted_mov[i] = reg.shift_frame(mov[i], -tvecs[i, 0].astype(int), -tvecs[i, 1].astype(int), cp = np)
...
def apply_plane_shiftd4D(mov, tvecs):
    ...
    for i in range(nt):
        ShiftedMov[:,i,:,:] = apply_plane_shifts3D(mov[:,i,:,:,], tvecs)

    return np.asarray(ShiftedMov)

##########################################
#reference image function cpu and gpu
def get_reference_img_gpu(frames, percent_contribute, niter, rmins = None, rmaxs = None, batch_size = 20, max_reg_xy = 50, sigma = [1.45, 0]):
    """
    ...
    for iter in range(niter):
        ...
        for z in range(nz): # need to go over each frame for the reference.. do sigz = 0 as well for this..
        ...
        for i in range(nBatches):
            idx1 = min(frames.shape[1], (i+1)*batch_size) #catch the case when the final idx would be larger than the array 
            tmp_frames, tmp_ymax, tmp_xmax, tmp_cmax = reg.rigid_2d_reg_gpu(frames[:,i*batch_size : idx1,:,:], mult_mask, add_mask, refs_f, max_reg_xy, rmins, rmaxs, crosstalk_coeff = None, shift=True) #  param rmins/rmaxs
            ...
            del tmp_frames; del tmp_ymax; del tmp_xmax; del tmp_cmax
        ...
        for z in range(nz):
            ...
            refImg[z] = frames[z,isort.squeeze(),:,:].mean(axis=0).squeeze() #refImg is mean of the most correlated frames
            refImg[z,:,:] = reg.shift_frame(refImg[z,:,:], int(n.round(+ymax[iter,z,isort].mean())), int(n.round(-xmax[iter, z,isort].mean())), cp = n) # ~recenter refImg

        used_frames.append(iter_frames)
    return refImg, ymax, xmax, cmax, used_frames

def get_reference_img_cpu(frames, percent_contribute, niter, rmins = None, rmaxs = None, max_reg_xy = 30, sigma = [1.45, 0]):
    """
    This function creates a reference image using the GPU for a speed up
    ...
    for iter in range(niter):
        ...
        for z in range(nz): # need to go iver each frame for the reference.. do sigz = 0 as well for this..
            refs_f[z] = phasecorr_ref(refImg[z,:,:].squeeze(), smooth_sigma = 1.15)

        #mask is applied in rigid_2d_reg_cpu
        tmp, ymax[iter], xmax[iter], cmax[iter] = reg.rigid_2d_reg_cpu(frames, mult_mask, add_mask, refs_f, max_reg_xy,
                        rmins, rmaxs, crosstalk_coeff = None, shift=True) # check max_reg_xy
        frames = tmp.swapaxes(0,1) #need to swap axes as by default reigid_2d_reg_cpu return nt,nz not nz,nt
        del tmp
...
def compute_reference_and_masks_3d(mov_cpu, reference_params, log_cb = default_log, rmins = None, rmaxs = None, use_GPU = True):
    percent_contribute = reference_params['percent_contribute']
    niter = reference_params['niter']
    sigma = reference_params['sigma']
...
def  get_reference_img_gpu_3d(mov_cpu, percent_contribute, niter, xpad, ypad, rmins = None, rmaxs = None, batch_size = 20, pc_size = (2, 20, 20), sigma = (0, 1.5), log_cb = default_log):
    ...
    for iter_idx in range(niter):
        ...
        for t in range(nt):
            cmax[iter_idx, t] = phase_corr_shifted[t, pc_peak_loc[t,0], pc_peak_loc[t,1], pc_peak_loc[t,2]]
