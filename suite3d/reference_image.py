...
def compute_reference_and_masks(mov_fuse, reference_params, log_callback = default_log, rmins = None, rmaxs = None, use_GPU = True):
    ...
    if use_GPU:
        ref_image, ymax, xmax, cmax, used_frames = \
            get_referance_img_gpu(mov_fuse, percent_contribute, niter, rmins = rmins, rmaxs = rmaxs, batch_size = batch_size, max_reg_xy = max_reg_xy, sigma = Sigma)
    else:
        ref_image, ymax, xmax, cmax, used_frames = \
            get_referance_img_cpu(mov_fuse, percent_contribute, niter, rmins = rmins, rmaxs = rmaxs,  max_reg_xy = max_reg_xy, sigma = Sigma)
    
    #allign the reference image
    uncorrected_tvecs = register_planes(ref_image, reference_params)
    ...
    if reference_params.get('norm_frames', False):
        ...
        all_refs_and_masks = get_phasecorr_and_masks(clipped_ref_img, reference_params)
    else:
        all_refs_and_masks = get_phasecorr_and_masks(ref_image, reference_params)
...
def register_planes(mov3D, reference_params):
    """ 
...
def get_referance_img_cpu(frames, percent_contribute, niter, rmins = None, rmaxs = None, max_reg_xy = 30, sigma = [1.45, 0]):
    """
    This function creates a reference image using the GPU for a speed up
