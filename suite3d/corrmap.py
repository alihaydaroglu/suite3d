...
def calculate_corrmap(mov, params, summary=None, batch_dir=None, 
                      mov_sub_dir=None, iter_limit = None, log=default_log):
    '''
    ...
    log("Computing correlation map of movie with %d frames, volume shape: %d, %d, %d" % \
          (nt, nz, ny, nx), 1)
    ...
    if dir is None:
        ...
        mov_sub_paths = make_batch_paths(mov_sub_dir, n_batches,prefix='mov_sub',
                                         suffix='.npy', dirs=False)
    ...
    for batch_idx in range(n_batches):
        ...
        vmap_batch, mov_sub_batch = compute_corr_map_batch(mov_batch, corr_map_params,
                                                           computation_params, accums,
                                                           summary, log)
        if save:
            # save results to previously created dirs
            save_batch_results(vmap_batch, accums,
...
def compute_corr_map_batch(mov, corr_map_params = None, 
    ...
    sdmov = dtu.accumulate_sdmov(accum['sdmov_2'], mov, nt, )
